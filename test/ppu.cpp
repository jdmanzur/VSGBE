...
//Construtor
Ppu::Ppu(Mmu* mem_s) : video(640,576)
{
...
void Ppu::drawnLineBG()
{
	//Deslocamento horizontal e vertical para scroll
        ...
        //tiles deslocados em relação ao scroll
        uint8_t inc_tile_y = (scrolly/8);

        //Linhas deslocadas dentro do tile pelo scroll
        uint8_t inc_line_y = (scrolly & 0x07);

        //No do primero sprite a ser desenhado
        uint16_t n_tile = ((((this->line/8)*32) + (inc_tile_y * 32)) & 0x03FF);
        ...
        for(int i = 0; i < 20; i++)
        {
                //std::cout << (int)(n_tile+i) << " ";
                //Pega os dados do tile
                this->GetTileLine((n_tile+i),sub_line,tile_data);
                
                this->video.drawnTileLine(tile_data,(*mem).read(BGP),this->line,(i*8));
        ...
        while((i < 40) && (ob_num < 10))
        {
                ...
                if((obj_x < 160)&&(obj_y < 144)&&(this->line >= obj_y))
                        ...
                        if((this->line - obj_y) <= ob_size)
                        {
                                ...
                                video.drawnTileLine(obj_data,(*mem).read(OBP0),this->line,obj_x);
                        }
...
void Ppu::GetObjLine(uint8_t obj_n,uint8_t line,uint8_t* obj_data)
{
	...
	uint16_t data_addr = 0x8000 + ((*mem).read(oam_addr + 2) * 16);
        ...
        uint8_t y_flip = ((*mem).read(data_addr + 3) & 0x40) ? 1 : 0;
        //Verifica rotação eixo x
        uint8_t x_flip = ((*mem).read(data_addr + 3) & 0x20) ? 1 : 0;
...
void Ppu::GetTileLine(uint16_t tile_n,uint8_t line,uint8_t* tile_data)
{
	...
	uint16_t data_addr = ((*mem).read(LCDC) & 0x10) ? 0x8000 : 0x8800;
        ...
        //Endereço do tile
        uint16_t tile_addr = data_addr + ((*mem).read(code_addr + tile_n)*16);
        ...
        for(int j = 0; j < 8; j++)
                //TODO:Melhorar, código pouco legível
                //tile_data[7-j] = (((*mem)[base]&(1<<j))>>j)|(((*mem)[base+1]&(1<<j))>>((line * 2)-j));
        ...
        //Grava a linha na memória
        for(int i = 0; i < 8; i++)
                tile_data[i] = tile[line][i];
